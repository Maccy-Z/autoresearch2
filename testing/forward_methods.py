...
from backward_method import BACKWARD_METHODS
...
DEFAULT_BLOCK_M = 64
DEFAULT_BLOCK_N = 64
BACKWARD_IMPL = BACKWARD_METHODS["direct"]
...
def dense_to_tilesparse_direct(
    dense: torch.Tensor,
...
def dense_to_tilesparse_disabled(
    dense: torch.Tensor,
    sparse_data: tuple[torch.Tensor, torch.Tensor],
    BLOCK_M: int = DEFAULT_BLOCK_M,
    BLOCK_N: int = DEFAULT_BLOCK_N,
) -> BitsparseTensor:
    return dense_to_tilesparse_direct(dense, sparse_data, BLOCK_M, BLOCK_N)


@custom_op("bitsparse_forward_methods::dense_to_tilesparse_pack", mutates_args={"vals", "offset"})
def dense_to_tilesparse_pack_op(
    dense: torch.Tensor,
    vals: torch.Tensor,
    offset: torch.Tensor,
    BLOCK_M: int,
    ...
    return _dense_to_tilesparse_pack_impl(dense, vals, offset, BLOCK_M, BLOCK_N)


@dense_to_tilesparse_pack_op.register_fake
def _(
    dense: torch.Tensor,
    vals: torch.Tensor,
    offset: torch.Tensor,
    ...
    return (
        torch.empty(num_tiles * TILE_BYTES, device=dense.device, dtype=torch.uint8),
        torch.empty(num_tiles + 1, device=dense.device, dtype=torch.int32),
        torch.empty_like(offset),
    )


def dense_to_tilesparse_custom_pack(
    dense: torch.Tensor,
    sparse_data: tuple[torch.Tensor, torch.Tensor],
    BLOCK_M: int = DEFAULT_BLOCK_M,
    BLOCK_N: int = DEFAULT_BLOCK_N,
    ...
    vals, offset = sparse_data
    bitmask, prefix, vals_offset = dense_to_tilesparse_pack_op(
        dense, vals, offset, BLOCK_M, BLOCK_N
    )
    return _make_bitsparse(vals, bitmask, prefix, vals_offset, dense.shape, BLOCK_M, BLOCK_N)
...
class FFNSparseDirect(Function):
    ...
    def forward(ctx, x, W1, W2, sparse_data):
        ...
        z_sparse = dense_to_tilesparse_direct(preact, sparse_data)
...
class FFNSparseDisabledPack(Function):
    """Forward with dense_to_tilesparse behind a torch.compiler.disable graph break."""

    @staticmethod
    def forward(ctx, x, W1, W2, sparse_data):
        preact = x @ W1.T
        preact.relu_()
        z_sparse = dense_to_tilesparse_disabled(preact, sparse_data)
        ctx.z_sparse = z_sparse
        ctx.save_for_backward(x, W1, W2)
        return preact @ W2.T
...
class FFNSparseCustomPack(Function):
    """Forward with only dense_to_tilesparse hidden behind an opaque custom op."""

    @staticmethod
    def forward(ctx, x, W1, W2, sparse_data):
        preact = x @ W1.T
        preact = F.relu(preact)
        z_sparse = dense_to_tilesparse_custom_pack(preact, sparse_data)
        ctx.z_sparse = z_sparse
        ctx.save_for_backward(x, W1, W2)
        return preact @ W2.T
...
FORWARD_METHODS = {
    "direct": FFNSparseDirect,
    "disabled_pack": FFNSparseDisabledPack,
    "custom_pack": FFNSparseCustomPack,
    "custom_ffn": FFNSparseCustomFFN,
}


DENSE_TO_TILESPARSE_METHODS = {
    "direct": dense_to_tilesparse_direct,
    "disabled": dense_to_tilesparse_disabled,
    "custom_pack": dense_to_tilesparse_custom_pack,
}
